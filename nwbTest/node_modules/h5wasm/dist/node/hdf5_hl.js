...
exports.Dataset = exports.File = exports.Group = exports.ACCESS_MODES = exports.ready = exports.FS = exports.Module = void 0;
const hdf5_util_js_1 = __importDefault(require("./hdf5_util.js"));
exports.default = exports.Module;
exports.FS = null;
const ready = hdf5_util_js_1.default({ noInitialRun: true }).then(result => { exports.Module = result; exports.FS = exports.Module.FS; });
exports.ready = ready;
exports.ACCESS_MODES = {
    "r": "H5F_ACC_RDONLY",
    "a": "H5F_ACC_RDWR",
    "w": "H5F_ACC_TRUNC",
    "x": "H5F_ACC_EXCL",
...
function get_attr(file_id, obj_name, attr_name) {
    let metadata = exports.Module.get_attribute_metadata(file_id, obj_name, attr_name);
    let nbytes = metadata.size * metadata.total_size;
    let data_ptr = exports.Module._malloc(nbytes);
    var processed;
    try {
        exports.Module.get_attribute_data(file_id, obj_name, attr_name, BigInt(data_ptr));
        let data = exports.Module.HEAPU8.slice(data_ptr, data_ptr + nbytes);
        processed = process_data(data, metadata);
    }
    finally {
        if (metadata.vlen) {
            exports.Module.reclaim_vlen_memory(file_id, obj_name, attr_name, BigInt(data_ptr));
        }
        exports.Module._free(data_ptr);
    }
    return processed;
}
function process_data(data, metadata) {
    ...
    if (metadata.type == exports.Module.H5T_class_t.H5T_STRING.value) {
        if (metadata.vlen) {
            let output = [];
            let reader = (metadata.cset == 1) ? exports.Module.UTF8ToString : exports.Module.AsciiToString;
    ...
    else if (metadata.type == exports.Module.H5T_class_t.H5T_INTEGER.value) {
    ...
    else if (metadata.type == exports.Module.H5T_class_t.H5T_FLOAT.value) {
        let accessor_name = "Float" + ((metadata.size) * 8).toFixed() + "Array";
        if (accessor_name in globalThis) {
            data = new globalThis[accessor_name](data.buffer);
        }
    }
    else if (metadata.type == exports.Module.H5T_class_t.H5T_COMPOUND.value) {
    ...
    else if (metadata.type == exports.Module.H5T_class_t.H5T_ARRAY.value) {
...
class HasAttrs {
    get attrs() {
        let attr_names = exports.Module.get_attribute_names(this.file_id, this.path);
        let attrs = {};
        for (let name of attr_names) {
            let metadata = exports.Module.get_attribute_metadata(this.file_id, this.path, name);
    ...
    create_attribute(name, data, shape = null, dtype = null) {
        ...
        if (metadata.vlen) {
            exports.Module.create_vlen_str_attribute(this.file_id, this.path, name, prepared_data, shape.map(BigInt), metadata.type, metadata.size, metadata.signed, metadata.vlen);
        }
        else {
            let data_ptr = exports.Module._malloc(prepared_data.byteLength);
            try {
                exports.Module.HEAPU8.set(prepared_data, data_ptr);
                exports.Module.create_attribute(this.file_id, this.path, name, BigInt(data_ptr), shape.map(BigInt), metadata.type, metadata.size, metadata.signed, metadata.vlen);
            }
            finally {
                exports.Module._free(data_ptr);
            }
...
class Group extends HasAttrs {
    ...
    keys() {
        return exports.Module.get_names(this.file_id, this.path);
    }
    *values() {
    ...
    *items() {
    ...
    get_type(obj_path) {
        return exports.Module.get_type(this.file_id, obj_path);
    }
    get(obj_name) {
        let fullpath = (/^\//.test(obj_name)) ? obj_name : this.path + "/" + obj_name;
        fullpath = normalizePath(fullpath);
        let type = this.get_type(fullpath);
        if (type == exports.Module.H5O_TYPE_GROUP) {
            return new Group(this.file_id, fullpath);
        }
        else if (type == exports.Module.H5O_TYPE_DATASET) {
            return new Dataset(this.file_id, fullpath);
        }
    }
    create_group(name) {
        exports.Module.create_group(this.file_id, this.path + "/" + name);
    ...
    create_dataset(name, data, shape = null, dtype = null) {
        ...
        if (metadata.vlen) {
            exports.Module.create_vlen_str_dataset(this.file_id, this.path + "/" + name, prepared_data, shape.map(BigInt), metadata.type, metadata.size, metadata.signed, metadata.vlen);
        }
        else {
            let data_ptr = exports.Module._malloc(prepared_data.byteLength);
            try {
                exports.Module.HEAPU8.set(prepared_data, data_ptr);
                exports.Module.create_dataset(this.file_id, this.path + "/" + name, BigInt(data_ptr), shape.map(BigInt), metadata.type, metadata.size, metadata.signed, metadata.vlen);
            }
            finally {
                exports.Module._free(data_ptr);
            }
...
class File extends Group {
    constructor(filename, mode = "r") {
        super(null, "/");
        let access_mode = exports.ACCESS_MODES[mode];
        let h5_mode = exports.Module[access_mode];
        if (['H5F_ACC_RDWR', 'H5F_ACC_RDONLY'].includes(access_mode)) {
            // then it's an existing file...
            this.file_id = exports.Module.ccall("H5Fopen", "bigint", ["string", "number", "bigint"], [filename, h5_mode, 0n]);
        }
        else {
            this.file_id = exports.Module.ccall("H5Fcreate", "bigint", ["string", "number", "bigint", "bigint"], [filename, h5_mode, 0n, 0n]);
        }
        this.filename = filename;
        this.mode = mode;
    }
    flush() {
        exports.Module.flush(this.file_id);
    }
    close() {
        return exports.Module.ccall("H5Fclose", "number", ["bigint"], [this.file_id]);
    }
}
exports.File = File;
class Dataset extends HasAttrs {
    ...
    get metadata() {
        return exports.Module.get_dataset_metadata(this.file_id, this.path);
    }
    get dtype() {
    ...
    get value() {
        ...
        var processed;
        try {
            exports.Module.get_dataset_data(this.file_id, this.path, null, null, BigInt(data_ptr));
            let data = exports.Module.HEAPU8.slice(data_ptr, data_ptr + nbytes);
            processed = process_data(data, metadata);
        }
        finally {
            if (metadata.vlen) {
                exports.Module.reclaim_vlen_memory(this.file_id, this.path, "", BigInt(data_ptr));
            }
            exports.Module._free(data_ptr);
        }
        return processed;
    ...
    slice(ranges) {
        ...
        var processed;
        try {
            exports.Module.get_dataset_data(this.file_id, this.path, count, offset, BigInt(data_ptr));
            let data = exports.Module.HEAPU8.slice(data_ptr, data_ptr + nbytes);
            processed = process_data(data, metadata);
        }
        finally {
